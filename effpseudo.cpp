...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<lli> primefact = getPrimeFactorization2(n);
        ...
        map<lli, lli> primecnt;

        lli j = 0LL;
        lli c = 1LL;

        while ((j + 1) < primefact.size())
        {
            if (primefact[j] == primefact[j + 1])
            {
                c++;
            }
            else
            {
                primecnt[primefact[j]] = c;
                c = 1LL;
            }
            j=j+1;
        }

        //cout<<j<<" "<<primefact.size()<<'\n';
        if (j < primefact.size())
            primecnt[primefact[j]] = c;
        ...
        for (auto u : primecnt)
        {
            ...
            lli tmp = binExp(u.first, (lli)(u.second * m) + 1LL);

            tmp = (tmp + MOD - 1) % MOD;

            //lli a = InverseModuloPrime(u.first - 1LL, MOD);

            lli a=modInverse(u.first-1,MOD);
